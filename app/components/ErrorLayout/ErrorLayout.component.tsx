    ...
    <html lang="en">
      ...
      <body>
        <ChakraProvider theme={C3Theme}>
          <Fonts />
          <VStack margin={"auto"} minH="100vh" minW={{ base: "none", lg: "100ch" }} maxW={"150ch"}>
...
function ErrorLayout({ error, caught }: IErrorLayoutProps) {
  ...
  const text =
    caught?.status === 404 ? "We couldn't find what you were looking for." : "We weren't expecting this to happen";

  console.error(error);
  return (
    <Document cookies={""} ENV={""}>
      <Header />
      <Box h={"75vh"}>
        <Heading mb={6}>{title}</Heading>
        <Text mb={2}>{text}</Text>
        <Text mb={6}>Click on the link below to head home and try again.</Text>
