...
const PrayerCard = ({ node }: IPrayerCardProps) => {
	const [hasPrayed, setHasPrayed] = useState(0);

	const cardClasses =
		node.data.type === 'praise' ? 'bg-blue-400' : 'bg-blue-700';
	const buttonClasses =
		node.data.type === 'praise' ? 'bg-blue-700' : 'bg-blue-400';

	useEffect(() => {
		const pData = JSON.parse(
			window.localStorage.getItem(node.data.type) as string
		);
	...
	const onClick = () => {
		...
		window.localStorage.setItem(
			...
			JSON.stringify({
				...pData,
				[node.id]: newCount as unknown as string,
			})
		);
	};
	return (
